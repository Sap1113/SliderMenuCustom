package com.slider.slidermenucustom.uc;
...
public class MainLayout extends LinearLayout {
    ...
    private static int menuRightMargin = 500;
    ...
    protected void onLayout(boolean changed, int left, int top, int right,
                            int bottom) {
        // Log.d("MainLayout.java onLayout()", "left " + left + " top " + top +
        // " right " + right + " bottom " + bottom);
        // Log.d("MainLayout.java onLayout()", "getHeight " + this.getHeight() +
        // " getWidth " + this.getWidth());

        // True if MainLayout 's size and position has changed
        // If true, calculate child views size
        if (changed) {
            // Note: LayoutParams are used by views to tell their parents how
    ...
    private void adjustContentPosition(boolean isScrolling) {
        ...
        // Log.d("MainLayout.java adjustContentPosition()", "scrollerOffset " +
        // scrollerOffset);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (contentXOffset + diffX <= 0) {
                    ...
                    // Use diffX will make content cross the border, so only
                    // translate by -contentXOffset
            ...
            case MotionEvent.ACTION_UP:
                ...
                // Start scrolling
                // Remember that when content has a chance to cross left border,
                // lastDiffX is set to 0
                if (lastDiffX > 0) {
                    // User wants to show menu
                    currentMenuState = MenuState.SHOWING;

                    // No need to set to Visible, because we have set to Visible in
                    // ACTION_MOVE
                    // menu.setVisibility(View.VISIBLE);

                    // Log.d("MainLayout.java onContentTouch()",
                    // "Up contentXOffset " + contentXOffset);
